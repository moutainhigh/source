define(['jquery', 'layui', 'until'], function($, LAY, until){
	layui.use(['table', 'form', 'layer'], function(){
		 ...
		 if(i18n == 'en_US'){
				table.render({
				    ...
				    cols: [[
				      ...
				      { field:'actName', title: 'actName', sort: true, align: 'center', },
				      { field:'actType', title: 'actType', sort: true, align: 'center',templet:function(data){
							if(data.actType == 1){
								return "添加";
				      ...
				      { field:'actTime', title: 'actTime', sort: true, align: 'center' ,templet: function (data) {
		...
		//切换英文
			if(i18n == 'en_US'){
				$("#actTypeName").empty();
				$("#actTypeName").append('ActType');
				$("#actUsername").empty();
				$("#actUsername").append('UserName');
				$("#searchActDeily").empty();
				$("#searchActDeily").append('Search');
				$("#searchActUserName").attr("placeholder","UserName");
				$("#LicensePlate").empty();
