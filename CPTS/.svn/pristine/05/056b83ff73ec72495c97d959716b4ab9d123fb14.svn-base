	...
	layui.use(['table', 'form','laydate'], function () {
		...
		if(i18n == 'en_US'){
			table.render({
				...
				cols: [[
					{ field: 'companyName', title: 'companyName', sort: true,hide:type==1?false:true},
					{ field: 'bankNum', title: 'bankNum', sort: true,hide:type==1?false:true},
					{ field: 'bankHolder', title: 'bankHolder', sort: true,hide:type==1?false:true},
					...
					{ field: 'applyStatus', title: 'applyStatus', sort: true, align: 'center' , templet:function(data){
					...
					{ field: 'handleTime', title: 'handleTime', sort: true, align: 'center' ,templet:function(data){
					...
					{ field: 'moneyStatus', title: 'moneyStatus', sort: true, align: 'center', templet:function(data){
					...
					{ field: 'free', title: 'free', sort: true, align: 'center'},
					{ fixed : 'right', title : 'right', align : 'center',width:180, hide:type==1?false:true, templet:function(data){
						var button = "<button lay-event='details' class='layui-btn layui-btn-xs'>Bill</button>";
						if(data.applyStatus == 0){
							button += "<button class='layui-btn layui-btn-xs' lay-event='edit'>Agree</button> ";
						}else if(data.moneyStatus == 0){
							button += "<button class='layui-btn layui-btn-xs' lay-event='edit'>Determine</button> ";
						}else{
							button += "finish";
						};
						return button;
		...
		//切换英文
		if(i18n == 'en_US'){
			$("#withdrawCompanyName").empty();
			$("#withdrawCompanyName").append('CompanyName');
			$("#withdrawSearch").empty();
			$("#withdrawSearch").append('Search');
			$("#exportExcel").empty();
			$("#exportExcel").append('ExportExcel');
			$("#searchFinanceCompany").attr("placeholder","CompanyName");
		}
        ...
        function exportExcelCompany(data) {
        	console.log(data);
                window.location.href = contextPath + "/order/exportExcelCompany"
				...
				layer.confirm(msg, function (index1) {
					...
					layer.open({
						...
						success: function(layero, index){
							...
							$("#exportExcelCompany").click(function () {
								exportExcelCompany(data);
							});
						},
