...
//        dateFeeBegin.setTime(dateIn.getTime() + (feeModel.getFreeMinute() * ONE_MINUTE_TIME));//计算开始计费时间
        ...
        if (DateUtil.isSameDate(dateIn, dateOut)) {
            long hours = (dateOut.getTime() - dateIn.getTime() - feeModel.getFreeMinute() * ONE_MINUTE_TIME) / (ONE_HOUR_TIME);
            if ((dateOut.getTime() - dateIn.getTime() - feeModel.getFreeMinute() * ONE_MINUTE_TIME) % (ONE_HOUR_TIME) == 0) {
         ...
         */
            ...
            int stayDays = DateUtil.getDiffDays(dateIn, dateOut);//停车 日期间隔天数 , 这里肯定 大于等于 1

            double fee1 = getFee1(feeModel, dateIn);//第一天费用
            double fee2 = 0;
